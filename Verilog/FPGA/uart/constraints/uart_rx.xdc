set_property PACKAGE_PIN C4 [get_ports i_rx]
set_property PACKAGE_PIN V16 [get_ports o_rx_data_valid]
set_property IOSTANDARD LVCMOS33 [get_ports clk]
set_property IOSTANDARD LVCMOS33 [get_ports i_rx]
set_property IOSTANDARD LVCMOS33 [get_ports o_rx_data_valid]
set_property PACKAGE_PIN E3 [get_ports clk]
set_property PACKAGE_PIN H17 [get_ports {o_rx_byte[0]}]
set_property PACKAGE_PIN K15 [get_ports {o_rx_byte[1]}]
set_property PACKAGE_PIN J13 [get_ports {o_rx_byte[2]}]
set_property PACKAGE_PIN N14 [get_ports {o_rx_byte[3]}]
set_property PACKAGE_PIN R18 [get_ports {o_rx_byte[4]}]
set_property PACKAGE_PIN V17 [get_ports {o_rx_byte[5]}]
set_property PACKAGE_PIN U17 [get_ports {o_rx_byte[6]}]
set_property PACKAGE_PIN U16 [get_ports {o_rx_byte[7]}]
create_clock -period 10.000 -name clk -waveform {0.000 5.000} clk

set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {o_rx_byte[0]}]
